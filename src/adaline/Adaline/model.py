...
class Neuron:
  def __init__(self, fg, fixed_value, teta, wteta = rd.random()):
    self.__wi = []
    self.__initWi = []
    self.__epoch = 0
    self.__teta = teta
  ...
  def train(self, train_x:pd.DataFrame, train_y:pd.DataFrame, max_epochs = 0, lerning_rate = rd.random(), verbose= False):
    ...
    while((self.__epoch < max_epochs if max_epochs > 0 else True)):
      self.__epoch += 1
      results = self.__predict(train_x, expected_result)

      sumt = 0
      for i in range(len(expected_result)):
        sumt += (expected_result[i] - results[i]) ** 2

      finalResult = sumt / len(expected_result)

      mediumError.append(finalResult)

      if (len(mediumError) > 2):
        if abs(mediumError[len(mediumError) - 1] - mediumError[len(mediumError) - 2]) <= self.__fixed_value:
          break
  ...
  def __predict(self, train_x:pd.DataFrame, train_y:list = []):
    ...
    for i in range(len(train_x.values)):
      ...
      if len(train_y) > 0:
        ...
        self.__updateWi(self.__lerning_rate, train_y[i], result, values[i])
